...
class Casino(commands.Cog):
    ...
    async def _send_casinoboard(
        ...
        for user_id, data in all_members.items():
            ...
            values = {
                "wagered": source.get("wagered", data.get("total_wagered", 0)),
                "profit": source.get("paid", data.get("total_paid", 0))
                - source.get("wagered", data.get("total_wagered", 0)),
                "games": source.get("games", data.get("total_games", 0)),
                "wins": source.get("wins", data.get("wins", 0)),
                "losses": source.get("losses", data.get("losses", 0)),
                "payout": source.get("biggest_payout", data.get("biggest_payout", 0)),
            }
            if not values["games"]:
                continue
