...
class TCGTracker(commands.Cog):
    ...
    async def tcgcheck(self, ctx: commands.Context) -> None:
        ...
        Shows online availability, and if ZIP codes are configured, also
        shows nearby in-store inventory for each product.
        """
        ...
        for upc, product in products.items():
            ...
            found_any = await self._process_check_results(
                ctx.guild, upc, product, online_results, channel, role
            ) or found_any
            ...
            if zip_codes:
                for zip_code in zip_codes:
                    ...
                    if store_results:
                        await self._send_store_embed(ctx.channel, product, zip_code, store_results)
                    await asyncio.sleep(1)

            await asyncio.sleep(1)

        summary = (
            "✅ Check complete! Alerts sent for new online restocks."
            if found_any
            else "✅ Check complete. Nothing new in stock online right now."
        )
